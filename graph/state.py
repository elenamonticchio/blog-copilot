...
Mappa diretta sui campi richiesti dal PDF:
    ...
    planning info     -> suggested_topics, editorial_plan, current_post_index, ...
"""
...
class PlannedPost(TypedDict):
    ...
    justification: str    # Perche questo topic ora (obbligatorio per il prof)
...
class AgentState(TypedDict):
    ...
    # ------------------------------------------------------------------ #
    # PIANIFICAZIONE (Suggest + Planner node)                              #
    # ------------------------------------------------------------------ #
    # Topic proposti da suggest_topics (consumati dal planner)
    # [{"topic": ..., "post_type": ..., "reason": ...}]
    ...
    # Documenti recuperati dal RAG - base per grounding e citazioni (K-RAG).
    ...
    # Trace ReAct: {"thought": ..., "action": ..., "observation": ...}
    ...
    # Citazioni esplicite per il draft (requisito PDF)
    citations: list[dict]   # [{"title": ..., "url": ..., "source": ...}]

    # Claim chiave estratti dal contenuto (requisito KG: "key claims extracted")
    key_claims: list[str]
    ...
    # Bool pulito per gli edge condizionali (derivato da user_status)
    approved: bool
