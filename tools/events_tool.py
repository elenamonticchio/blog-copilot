"""
Events Finder (CUSTOM 4 - requisito PDF: post-type 'Upcoming events in your
town/region').

Pipeline a due stadi sopra Tavily:
  1) Tavily search (max_results=10, days=60, topic=news)
  2) grader LLM (self-RAG-style, OpenAI) su ogni risultato, per tenere
     solo articoli che descrivono davvero un evento del tipo richiesto
     nella location richiesta (scarta menzioni di passaggio, eventi di
     altre città, mostre d'arte invece di festival cinema, ecc.).

Senza questo tool il post-type 'events' del PDF resterebbe scoperto.
"""
import os
...
def find_local_events(location: str = "Roma", kind: str = "festival cinema") -> str:
    ...
    Usalo per costruire post di tipo 'events' sugli appuntamenti del mese
    nell'area del lettore.
    """
