"""
TVmaze tool (CUSTOM 3 - requisito PDF: 'verify accuracy' per le serie TV).

TVmaze e' un database specializzato in serie TV: schedule degli episodi,
canali di messa in onda, riassunti e date. API gratuita, senza chiave.
Complementa TMDb (che e' piu' generico) sul fronte dei calendari TV.
"""
import re
import requests
...
def tvmaze_show_info(title: str, with_episodes: bool = True) -> str:
    ...
    (in corso/conclusa), rating medio, generi, trama, e — opzionale —
    gli ultimi 3 episodi andati in onda e i prossimi 3 in programma.

    Usalo per VERIFICARE il calendario di una serie TV (fact-check)
    o per costruire specchietti riassuntivi sulle stagioni.
    """
    try:
        params = {"q": title}
        if with_episodes:
